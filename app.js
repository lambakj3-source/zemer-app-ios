...
function flatten(v,k){
  if(Array.isArray(v))return v.map(x=>normalize(x,k)).filter(Boolean);
  if(v&&typeof v==="object"){
    const arrays=[];
    for(const key of Object.keys(v)){
      if(Array.isArray(v[key]))arrays.push(...v[key]);
    }
    return arrays.map(x=>normalize(x,k)).filter(Boolean);
  }
  return [];
}
function podcastResults(cats){
  const out=[],seen=new Set();
  const add=(arr,kind)=>{if(!Array.isArray(arr))return;arr.forEach(x=>{const n=normalize(x,kind);if(!n)return;const key=idOf(n)||JSON.stringify([title(n),artist(n)]);if(seen.has(key))return;seen.add(key);out.push(n);});};
  for(const [key,val] of Object.entries(cats||{})){
    const k=String(key).toLowerCase();
    if(k.includes("podcast")||k.includes("episode")){
      if(Array.isArray(val))add(val,k.includes("episode")?"episodes":"podcasts");
      else if(val&&typeof val==="object"){
        for(const [sub,items] of Object.entries(val)){
          if(Array.isArray(items))add(items,String(sub).toLowerCase().includes("episode")?"episodes":"podcasts");
        }
      }
    }
  }
...
function renderSearch(data,q){const cats=data?.categories||{};const order=[["Songs",cats.songs],["Artists",cats.artists],["Albums",cats.albums],["Singles",cats.singles],["Videos",cats.videos],["Playlists",cats.playlists],["Podcasts",podcastResults(cats)]];let html="",total=0;for(const pair of order){const name=pair[0],items=flatten(pair[1],name.toLowerCase());if(!items.length)continue;total+=items.length;html+='<section class="section"><h2>'+esc(name)+'</h2>';items.forEach((x,i)=>{html+=itemRow(x,i);});html+='</section>';document.querySelectorAll(".section").length;}$("results").innerHTML=html;$("status").textContent=total?total+" results for “"+q+"”":"No results.";bindRows();}
